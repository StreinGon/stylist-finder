...
module.exports = {
  ...
  add: async (values) => {
    ...
    const entry = await Token.create(data);

    // Create relational data and return the entry.
    return Token.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Token.associations.map(a => a.alias));
    const data = _.omit(values, Token.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Token.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Token.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Token.attributes).reduce((acc, curr) => {
      switch (Token.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
